...
void Genetic::crossoverASSC(Individual &result, const Individual &parent1, const Individual &parent2) {
    ...
    if (segmentsEqual){
        ...
        // Génération aléatoire du premier point de coupe
        int firstPoint = distr(gen); // Utilisation de la distribution pour générer un point aléatoire
    ...
    while (cpt3 < segments.size()) {
        ...
        // Ajouter le client au segment courant si ce n'est pas déjà ajouté
    ...
    // Copier ou insérer les segments dans le résultat
    if (!insertionSegment) {
        // Mélanger les segments sélectionnés
        std::shuffle(selectedSegmentsList.begin(), selectedSegmentsList.end(), std::mt19937{std::random_device{}()});

        // Copier les segments sélectionnés dans le résultat
        ...
        // Précalcul des distances entre les clients
        std::vector<std::vector<double>> distances(params.nbClients + 1, std::vector<double>(params.nbClients + 1, 0.0));
        for (int i = 0; i <= params.nbClients; ++i) {
            for (int j = 0; j <= params.nbClients; ++j) {
        ...
        // Insérer les segments dans le résultat
        int lastInsertedClient = 0; // initialiser le dernier client inséré à l'entrepôt
        std::vector<bool> segmentSelected(selectedSegmentsList.size(), false);
        for (size_t i = 0; i < selectedSegmentsList.size(); ++i) {
            ...
            for (size_t j = 0; j < selectedSegmentsList.size(); ++j) {
                if (!segmentSelected[j]) {
                    double distance = distances[lastInsertedClient][selectedSegmentsList[j].front()];
            ...
            // Insérer les clients du segment sélectionné dans le résultat
            for (int client : selectedSegmentsList[nextSegmentIndex]) {
                if (!freqClient[client]) {
                    result.chromT.push_back(client);
                    freqClient[client] = true;
...
/*    
    // Selecting segments alternatively from parents and storing them in a list
    std::vector<std::pair<int, int>> selectedSegmentsList;
    for (int i = 0; i < numSegment; ++i) {
        int bestSegment = -1;
        double minDistance = std::numeric_limits<double>::max();
